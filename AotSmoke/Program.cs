...
static void CheckBlake3()
{
    ...
    Equal(expected, Blake3.Get256ByteArray(ReadOnlySpan<byte>.Empty), "BLAKE3 empty vector");
    using var hasher = Blake3Hasher.New();
    hasher.Update(ReadOnlySpan<byte>.Empty);
    var output = new byte[32];
    hasher.FinalizeHash(output);
    ...
    Equal(expected, Blake3.Get256ByteArray(data), "BLAKE3 1025-byte vector");
    hasher.Update(data);
    hasher.FinalizeHash(output);
    Equal(expected, output, "BLAKE3 incremental vector");
    Check(hasher.FinalizeHash().Equals(Blake3.Get256Struct(data)), "BLAKE3 struct output");

    hasher.Reset();
    hasher.UpdateParallel(data);
    hasher.FinalizeHash(output);
    Equal(expected, output, "BLAKE3 parallel update");
    var extended = new byte[4096];
    hasher.FinalizeHash(extended);
    Equal(expected, extended.AsSpan(0, 32), "BLAKE3 extended output");
    hasher.FinalizeHash(Span<byte>.Empty);

    ulong[] words = [1, 2, 3, 4];
    hasher.Reset();
    hasher.Update<ulong>(words);
    hasher.FinalizeHash(output);
    Equal(Blake3.Get256ByteArray(MemoryMarshal.AsBytes(words.AsSpan())), output, "BLAKE3 generic update");
    hasher.Reset();
    hasher.UpdateParallel<ulong>(words);
    hasher.FinalizeHash(output);
    Equal(Blake3.Get256ByteArray(MemoryMarshal.AsBytes(words.AsSpan())), output, "BLAKE3 generic parallel update");
    ...
    Throws<NullReferenceException>(() => uninitialized.FinalizeHash(), "BLAKE3 invalid state");
}
...
static void CheckSodium()
{
    ...
    CryptoDual.CreateKeyPair(secretKey, publicKey, aliceSecret, alicePublic);
    var recovered = new byte[32];
    CryptoDual.ConvertBoxPublicKeyToSign(alicePublic, recovered);
    ...
    Throws<CryptographicException>(() => CryptoPasswordHash.GetHashString("password", memoryLimit: 0), "Argon2id failure");
    ...
    Equal(Convert.FromHexString("0e5751c026e543b2e8ab2eb06099daa1d1e5df47778f7787faab45cdf12fe3a8"),
        Blake2B.Get256ByteArray(ReadOnlySpan<byte>.Empty), "BLAKE2b empty vector");
}
...
static void CheckManagedAlgorithms()
{
    ...
    Equal(SHA256.HashData(message), Sha2Helper.Get256ByteArray(message), "SHA-256");
    Equal(Convert.FromHexString("a7ffc6f8bf1ed76651c14756a061d662f580ff4de43b49fa82d80a4b80f8434a"),
        Sha3Helper.Get256ByteArray(ReadOnlySpan<byte>.Empty), "SHA3-256 empty vector");
    ...
    Equal(message, Hex.FromStringToByteArray(Hex.FromBytesToString(message)), "Hex");
    Equal(message, FastBase64.Decode(FastBase64.EncodeToString(message)), "Base64");
    Equal(message, FastBase64Url.Decode(FastBase64Url.EncodeToString(message)), "Base64Url");
    Equal(message, Base32Sort.Default.FromStringToByteArray(Base32Sort.Default.FromBytesToString(message)), "Base32");
    ...
    CheckPoint(Secp256k1Curve.Instance,
        "79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798",
        "483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8");
    CheckPoint(Secp256r1Curve.Instance,
...
static void CheckPoint(ECCurveBase curve, string xHex, string yHex)
{
    ...
    Equal(y, curve.DecompressY(x, curve.CompressY(y)), curve.CurveName);
}
