...
/// <see cref="ECCurveBase.DecompressY(ReadOnlySpan{byte}, uint)"/>.
...
/// <see cref="ECCurveBase.DecompressY(ReadOnlySpan{byte}, uint)"/>, which also allocates the
...
public class EcCompressYBenchmark
{
    ...
    public EcCompressYBenchmark()
    {
        ...
        // DecompressY returns null on a curve/length mismatch, and that failure path is far
        ...
        static void Verify(byte[]? actual, byte[] expected, string curveName)
        {
            ...
            {
                throw new InvalidOperationException($"{curveName}: CompressY/DecompressY did not round-trip.");
            }
        }
    }

    [Benchmark(Baseline = true, Description = "secp256r1 CompressY + DecompressY")]
    public byte[]? P256R1_CompressDecompress()
    {
        var yt = Secp256r1Curve.Instance.CompressY(this.r1Y);
        return Secp256r1Curve.Instance.DecompressY(this.r1X, yt);
    }

    [Benchmark(Description = "secp256k1 CompressY + DecompressY")]
    public byte[]? P256K1_CompressDecompress()
    {
        var yt = Secp256k1Curve.Instance.CompressY(this.k1Y);
        return Secp256k1Curve.Instance.DecompressY(this.k1X, yt);
    }
}
