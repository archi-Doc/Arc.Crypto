...
namespace Beta.Crypto
{
    /// <summary>
    /// FarmHash 64bit Class.
    /// Uses System.Numerics.BitOperation.
    /// </summary>
    public class FarmHash : Arc.Crypto.IHash
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static uint Rotate32(uint val, int shift) => BitOperations.RotateRight(val, shift); // shift == 0 ? val : ((val >> shift) | (val << (32 - shift)));
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong Rotate64(ulong val, int shift) => BitOperations.RotateRight(val, shift); // shift == 0 ? val : (val >> shift) | (val << (64 - shift));
