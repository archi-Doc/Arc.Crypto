...
public class RandomVault2 : RandomUInt64
{
    ...
    static RandomVault2()
    {
        ...
        Xoshiro = new RandomVault2(x => xo.NextBytes(x), false, 16);
        RandomNumberGenerator = new RandomVault2(x => System.Security.Cryptography.RandomNumberGenerator.Fill(x), true);
        Libsodium = new RandomVault2(x => CryptoRandom.NextBytes(x), true);
        var aegis = new AegisRandom();
        Aegis = new RandomVault2(x => aegis.NextBytes(x), false, 16);
    }

    /// <summary>
    ///  Gets the cryptographically secure pseudo random number pool (<see cref="AegisRandom"/>).
    /// </summary>
    public static RandomVault2 Aegis { get; }

    /// <summary>
    ///  Gets the pseudo random number pool (<see cref="Xoshiro256StarStar"/>).
    /// </summary>
    public static RandomVault2 Xoshiro { get; }

    /// <summary>
    ///  Gets the cryptographically secure pseudo random number pool (<see cref="RandomNumberGenerator.Fill(Span{byte})"/>).
    /// </summary>
    public static RandomVault2 RandomNumberGenerator { get; }

    /// <summary>
    ///  Gets the cryptographically secure pseudo random number pool (Libsodium: <see cref="CryptoRandom.NextBytes(Span{byte})"/>).
    /// </summary>
    public static RandomVault2 Libsodium { get; }
    ...
    public RandomVault2(Action<Span<byte>> nextBytes, bool isThreadSafe, int skipVaultThreshold = DefaultSkipVaultThreshold)
    {
